...
function corsOriginAllowlist() {
  ...
  const site = String(process.env.VITE_SITE_URL || process.env.SITE_URL || '')
    .trim()
    .replace(/\/$/, '')
  ...
  if (process.env.CORS_ORIGINS) {
    for (const part of String(process.env.CORS_ORIGINS).split(',')) {
...
export function createApp() {
  ...
  app.get('/api/health', (_req, res) => {
    res.json({ ok: true, service: 'noah-payments', runtime: 'vercel' })
  })

  /**
   * Admin session check — security tests (customer → 403).
   * Catalog/admin CRUD remains Firestore client + security rules.
   */
    ...
    try {
      ...
      if (order.razorpayOrderId && order.razorpayOrderId !== razorpay_order_id) {
        throw publicError(
          400,
          'order_mismatch',
          'Razorpay order does not match this ecommerce order.',
        )
      }
...
async function handleWebhookEvent(event) {
  ...
  if (
    ...
    if (
      !isSuccessfulPaymentStatus(paymentEntity.status) &&
