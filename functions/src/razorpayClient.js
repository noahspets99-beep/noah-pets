...
export function getRazorpayCredentials() {
  ...
  if (!keyId || isUnsetOrPlaceholder(keyId)) {
    throw publicError(
      ...
      'Payment service is not configured. Set RAZORPAY_KEY_ID in functions/.env and restart the payments server.',
    )
  }
  ...
  if (!keySecret || isUnsetOrPlaceholder(keySecret)) {
    throw publicError(
      503,
      'payments_unconfigured',
      'Payment service is not configured. Set RAZORPAY_KEY_SECRET in functions/.env (not a VITE_ variable) and restart the payments server.',
    )
  }
